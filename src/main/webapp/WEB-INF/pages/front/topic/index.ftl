...
<div class="panel panel-default">
    <div class="panel-body">
        ...
        <div style="font-size: 12px;color: #838383; padding-top: 5px;">
            ...
            <span>
                &nbsp;•&nbsp;<a href="${path!}/?tab=${topic.tab}">${topic.sectionName!}</a>
            </span>
            &nbsp;
            <#if session.user?? && topic.author_id == session.user.id>
                <span>
                    <a href="${path!}/topic/edit/${topic.id!}"><span class="glyphicon glyphicon-edit"></span></a>&nbsp;
                    <a href="javascript:if(confirm('确定要删除此话题吗？'))location='${path!}/topic/delete/${topic.id!}'"><span
    ...
    <div class="panel-body" style="border-top: 1px #E5E5E5 solid; padding-top: 10px">
        ...
        <div class="pull-left topic-label">
            <#list labels as label>
                <a href="${path!}/?l=${label.id!}" style="text-decoration: none;">
    ...
    <#if session.user??>
        <div class="panel-footer" id="topic_footer">
            ...
            <input type="button" class="btn btn-xs btn-default" value="微博分享"
                onclick="window.open('http://service.weibo.com/share/share.php?url=${path!}/topic/${topic.id!}.html?r=${session.user.nickname!}&title=JFinalbbs%20-%20${topic.title!}', '_blank', 'width=550,height=370'); recordOutboundLink(this, '微博分享', 'weibo.com');"/>
...
<#if session.user??>
    <div class="panel panel-default" id="reply_input">
        ...
        <div class="panel-body">
            <form action="${path!}/reply/${topic.id!}" method="post" id="reply_form">
...
<script type="text/javascript">
    ...
    function collect(type, tid) {
        var url = "${path!}/collect/" + tid;
        if (_type == 0) _type = type;
        if (_type == 2) url = "${path!}/collect/delete/" + tid;
        $.ajax({
            url: url,
            async: false,
            cache: false,
