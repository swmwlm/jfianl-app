...
import com.shoukeplus.jFinal.common.model.base.BaseNotification;
...
public class Notification extends BaseNotification<Notification> {
	...
	public Page<Notification> paginate(int pageNumber, int pageSize, String authorId) {
		return super.paginate(pageNumber, pageSize, "select n.*, u.nickname ",
				"from sk_notification n " +
						"left join sk_user u on u.id = n.from_author_id " +
	...
	public List<Notification> findReadByAuthorId(String authorId, int size) {
		return super.find("select n.*, u.nickname from sk_notification n " +
				"left join sk_user u on u.id = n.from_author_id " +
	...
	public int updateNotification(String uid) {
		return Db.update("update sk_notification n set n.read = 1 where n.author_id = ?", uid);
	}

	//获取未读消息的数量
	public int countNotRead(String uid) {
		return super.find("select n.id as not_read_count from sk_notification n where n.read = 0 and n.author_id = ?", uid).size();
	}
}
