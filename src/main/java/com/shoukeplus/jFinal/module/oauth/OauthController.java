package com.shoukeplus.jFinal.module.oauth;
...
public class OauthController extends BaseController {
    ...
    public void qqlogincallback() throws QQConnectException, IOException {
        ...
        if (accessTokenObj.getAccessToken().equals("")) {
            ...
            if (user == null) {
                user = User.dao.findByOpenID(openID, "qq");
                ...
                User user1 = User.dao.findByOpenID(openID, "qq");
    ...
    public void weibologincallback() throws WeiboException, IOException {
        ...
        if (!StrUtil.isBlank(error) && error.equals("access_denied")) {
            ...
            if (weiboUser != null) {
                User user = (User) getSession().getAttribute(AppConstants.USER_SESSION);
                if (user == null) {
                    user = User.dao.findByOpenID(weiboUser.getId(), "sina");
                    String id = StrUtil.getUUID();
                    if (user == null) {
                        user = new User();
                        user.set("id", id)
                    ...
                    User user1 = User.dao.findByOpenID(weiboUser.getId(), "sina");
                    if (user1 != null && !user1.get("id").equals(user.get("id"))) {
                        getResponse().setCharacterEncoding("utf-8");
